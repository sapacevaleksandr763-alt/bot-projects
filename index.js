...
async function _callClaudeInner(prompt, sessionId, { onText } = {}) {
...
userMemory[sessionId].push({ role: "assistant", content: data.choices[0].message.content });
if (userMemory[sessionId].length > 20) {
userMemory[sessionId] = userMemory[sessionId].slice(-20);
}
 return { text: (data.choices && data.choices[0] && data.choices[0].message && data.choices[0].message.content) || "Нет ответа от модели", 
...
console.log("COMMIT START", userId);
if (!projectMemory[userId] || projectMemory[userId].data.length === 0) {
console.log("NO PROJECT DATA", userId);
return ctx.reply("Нет активного проекта");
}


const path = "./projects/project_" + userId + ".txt";

writeFileSync(path, projectMemory[userId].data.join("\n"));

await ctx.reply("Проект сохранен (commit)");
});
...
if (text.toLowerCase().includes("начать проект")) {
projectMemory[userId] = {
active: true,
data: []
};
console.log("PROJECT STARTED", userId);
}

if (projectMemory[userId] && projectMemory[userId].active) {
projectMemory[userId].data.push(text);
console.log("PROJECT SAVE:", text);
}
if (text.toLowerCase().includes("меня зовут")) {
const name = text.split("меня зовут")[1]?.trim();
if (name) {
userProfile[userId].name = name;
}
}
if (!userMemory[sessionId]) {
userMemory[sessionId] = [];
...
userMemory[sessionId].push({ role: "user", content: text });
if (userMemory[sessionId].length > 20) {
userMemory[sessionId] = userMemory[sessionId].slice(-20);
}
...
async function handleTextMessage(ctx, text) {
  ...
  try {
    ...
    const sessionId = userId;
    let profileText = "";
if (userProfile[sessionId]?.name) {
profileText += "Имя пользователя:"  + userProfile[sessionId].name + "\n";
}
    
    const history = (userMemory[sessionId] || [])
.map(m => m.role + ": " + m.content)
.join("\n");
    
    let result = await callClaude(profileText + history + "\n" + enrichedPrompt, sessionId)
